...
def number_of_lit(lights, instructions_ary)
...
def perform(instructions_ary, lights)
  instructions_from(instructions_ary).each do |instruction|
    puts "Performed #{instruction[:command]}: At the end we have #{lights.sum} lit lights!"
...
puts "After following all the instructions I ended up with #{number_of_lit(lights, instructions_ary)} lit 💡!"
