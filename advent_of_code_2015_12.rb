...
instructions = "turn on 887,9 through 959,629
...
turn off 853,861 through 924,964while
...
INSTRUCTION_PATTERN = /(?<raw_command>.+?)\s(?<s_row>\d+?),(?<s_col>\d+?)\sthrough\s(?<e_row>\d+?),(?<e_col>\d+)/

lights = Matrix.zero(1000)
instructions_ary = instructions.split("\n")

def brightness_of(lights, instructions_ary)
  #perform all the instructions on the light Matrix
  # sum all the elements -> as 1 stands for "turned on" and 0 for "turned off" the sum of all elements in the final lights matrix is the number of lit lights
  perform(instructions_ary, lights).sum
end

def perform(instructions_ary, lights)
  instructions_from(instructions_ary).each do |instruction|
    puts "Performed #{instruction[:command]}: At the beginning we had #{lights.sum} brightness!"
    instruction[:starting_col].upto(instruction[:end_col]) do |col|
      instruction[:starting_row].upto(instruction[:end_row]) do |row|
        #call the method with args (lights, row, col) that matches the command in the instraction
        send(instruction[:command].to_sym, lights, row, col)
      end
    end
  end
  lights
...
def instructions_from(instructions_ary)
  instructions_ary.each_with_object([]) do |instruction, a|
    a << {command: command_from(instruction[INSTRUCTION_PATTERN, :raw_command]), starting_row: instruction[INSTRUCTION_PATTERN, :s_row].to_i, starting_col: instruction[INSTRUCTION_PATTERN, :s_col].to_i, end_row: instruction[INSTRUCTION_PATTERN, :e_row].to_i, end_col: instruction[INSTRUCTION_PATTERN, :e_col].to_i}
  end
end

def command_from(raw_command)
  raw_command.gsub(" ", "_")
end

puts "After following all the instructions I ended up with #{brightness_of(lights, instructions_ary)} brighness 💡!"
